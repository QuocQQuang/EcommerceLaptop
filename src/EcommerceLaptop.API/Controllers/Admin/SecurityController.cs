...
public class SecurityController : ControllerBase
{
    ...
    [RequireAdminPermission(AdminPermissions.SecurityRead)]
    public async Task<IActionResult> GetRateLimitViolations([FromQuery] int hours = 24)
    {
        var from = DateTime.UtcNow.AddHours(-hours);
        var result = await _rateLimitingService.GetViolationsAsync(null, from, null);
    ...
    {
        var page = (skip / take) + 1;
        var result = await _auditLoggingService.SearchAuditLogsAsync(
        ...
        return Ok(new
        {
            ...
            page = page,
            pageSize = take,
            totalPages = (int)Math.Ceiling((double)estimatedTotal / take)
        });
    }
    ...
    public async Task<IActionResult> GetUserActivity(string userId, [FromQuery] int days = 30)
    {
        ...
        var result = await _auditLoggingService.GetUserAuditLogsAsync(userIdInt, fromDate);
    ...
    /// </summary>
    [HttpGet("audit-logs/admin/{adminUserId}")]
    [RequireAdminPermission(AdminPermissions.SecurityRead)]
    public async Task<IActionResult> GetAdminActivity(string adminUserId, [FromQuery] int days = 30)
    {
        if (!int.TryParse(adminUserId, out int adminIdInt))
            return BadRequest(new { error = "Invalid admin ID" });

        var fromDate = DateTime.UtcNow.AddDays(-days);
        var result = await _auditLoggingService.GetAdminAuditLogsAsync(adminIdInt, fromDate);
    ...
    [RequireAdminPermission(AdminPermissions.SecurityRead)]
    public async Task<IActionResult> GetAuditStatistics([FromQuery] int days = 7)
    {
        var fromDate = DateTime.UtcNow.AddDays(-days);
        var toDate = DateTime.UtcNow;
        var result = await _auditLoggingService.GetActivitySummaryAsync(fromDate, toDate);
